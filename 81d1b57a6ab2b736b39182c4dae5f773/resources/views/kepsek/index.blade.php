...
<div class="col-sm-12">
    <div class="panel panel-default panel-table">
      <div class="panel-heading">Daftar Post
        <div class="pull-right"><a class="btn btn-success" href="{{URL::to('/admin/kepsek/create')}}"><p class="glyphicon">+ <b>Tambah</b></p></a></div>
      </div>
        ...
        <table class="table table-striped table-hover">
          ...
          <tbody>
            <tr>
              ...
              <td>{{$row->post_kepsek}}</td>
              <td>{{$row->role}}</td>
              {{-- <td>{{$row->created_at->toFormattedDateString()}}</td> --}}
              <td><form action="{{ route('kepsek.destroy', $row->id) }}" method="post">
                {{ csrf_field() }}
                <input name="_method" type="hidden" value="DELETE">
                <a href="{{route('kepsek.edit', $row->id)}}" class="btn btn-warning"><i class="glyphicon glyphicon-edit"></i></a>
