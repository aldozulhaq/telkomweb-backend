...
<div class="col-sm-12">
    <div class="panel panel-default panel-table">
      <div class="panel-heading">Daftar Post
        <div class="pull-right"><a class="btn btn-success" href="{{URL::to('/visim/create')}}"><p class="glyphicon">+ <b>Tambah</b></p></a></div>
      </div>
      <div class="panel-body">
        <table class="table table-striped table-hover">
          ...
          @foreach ($visim as $row)
          <tbody>
            <tr>
              <td class="user-avatar"> <img src="{{ asset('assets/img/avatar6.png')}}" alt="Avatar">{{$row->user}}</td>
              <td>{{$row->post_visim}}</td>
              <td>{{$row->role}}</td>
              {{-- <td>{{$row->created_at->toFormattedDateString()}}</td> --}}
              <td><form action="{{ route('visim.destroy', $row->id) }}" method="post">
                {{ csrf_field() }}
                <input name="_method" type="hidden" value="DELETE">
                <a href="{{route('visim.edit', $row->id)}}" class="btn btn-warning"><i class="glyphicon glyphicon-edit"></i></a>
